defmodule PapyrusCollab.SharedWorkbooks do
  ...
  def delete_workbook(token, workbook_id)
    ...
    Store.delete_workbook(token, workbook_id)
  end

  def delete_workbook(_token, _workbook_id), do: {:error, :invalid_workbook_id}

  @spec read_workbook(String.t(), String.t()) :: {:ok, map() | nil} | {:error, term()}
  def read_workbook(token, workbook_id)
      when is_binary(token) and byte_size(token) > 0 and is_binary(workbook_id) and
             byte_size(workbook_id) > 0 do
    Store.read_workbook(token, workbook_id)
  end

  def read_workbook(_token, _workbook_id), do: {:error, :invalid_workbook_id}
  ...
  def sync_workbook(owner_id, token, workbook)
      when is_binary(owner_id) and byte_size(owner_id) > 0 and is_binary(token) and
             byte_size(token) > 0 and is_map(workbook) do
    Store.sync_workbook(owner_id, token, workbook)
  end

  def sync_workbook(_owner_id, _token, _workbook), do: {:error, :invalid_workbook_payload}
end
