defmodule PapyrusCollab.CloudWorkbooks.Store do
  ...
  @callback list_workbooks(String.t()) :: {:ok, [map()]} | {:error, term()}
  @callback read_workbook(String.t(), String.t()) ::
              {:ok, map() | nil} | {:error, term()}
  @callback write_workbook(String.t(), map(), String.t()) ::
              {:ok, map()} | {:error, term()}
  ...
  @spec write_workbook(String.t(), map(), String.t()) ::
          {:ok, map()} | {:error, term()}
  def write_workbook(user_id, workbook, client_id)
      when is_binary(user_id) and is_map(workbook) and is_binary(client_id) do
    adapter().write_workbook(user_id, workbook, client_id)
  end
